...
in~\cite{dejong-ho,ho-lieblich,antieau-auel,huybrechts-mattei}. The problem of determining
...
\begin{enumerate}
    ...
    \item[(d)] Over a number field, every central simple algebra of degrees $d=2,\ldots,10$ or $d=12$ is split
        by a genus $1$ curve~\cite{antieau-auel} (with the assumption that $k$ contains a primitive $8$th root of unity
        if $d=8$, an element of the form $\zeta_9+\zeta_9^{-1}$, where $\zeta_9$ is a primitive
        $9$th root of unity, if $d=9$, and a primitive $4$th root of unity if $d=12$).
...
In fact, our method works for central simple algebras of more general square-free integer degrees $N$, which we
