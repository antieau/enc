...
\begin{definition}
    If $V$ is a smooth, geometrically integral scheme over a field $k$, we let $\Br_{\nr}(V)$ denote 
    the subgroup of $\Br(V)$ consisting of classes which extend to a smooth proper model of $V$. We
    let $\Br_1(V)$ be the subgroup of $\Br(V)$ of classes which vanish after extension of scalars to
    the algebraic closure of $k$. And, we let $\Br_{1,\nr}(V)=\Br_\nr(V)\cap\Br_1(V)$, the subgroup
    of $\Br(V)$ of geometrically trivial unramified Brauer classes. This group contains as a
    subgroup the group $\Br_0(V)=\im(\Br(k)\rightarrow\Br(V))$.
\end{definition}

\begin{proposition}
    Let $E$ be an elliptic curve over a field $k$ and let $N$ be an integer prime to the
    characteristic of $k$. Let $V_E$ be the
    ...
    *\in\bZ/N\right\},$$ then, $$\Br(k)\rightarrow\Br_{1,\nr}(V_E)$$
    is an isomorphism.
\end{proposition}
...
\begin{proof}
    ...
    $\Br(k)\rightarrow\Br_{1,\nr}(V_E)$ is injective with image $\Br_0(V)$. We use the criterion of
    Harpaz--Wittenberg~\cite{harpaz-wittenberg-massey}.
\end{proof}
