...
\begin{document}
	...
	\end{proposition}
	\textbf{The proof under assumption 1 contains a gap.}
	\begin{proof}
		In view of Lemma \ref{injective-res}, it suffices to show that the assumptions imply the injectivity of $H^1(k,H^{\mathrm{ab}}_{\Bar{v}})\to H^1(k',H^{\mathrm{ab}}_{\Bar{v}})$. By a restriction-corestriction argument, (2) implies the injectivity. For (1), by the inflation-restriction sequence it suffices to show that $H^1(k'/k,(H^{\mathrm{ab}}_{\Bar{v}})^{\Gamma_{k'}})=0$. However, even if $E[N](k')=\{0\}$, we do not necessarily have $(H^{\mathrm{ab}}_{\Bar{v}})^{\Gamma_{k'}}=0$. The Galois modules $H^{\mathrm{ab}}_{\Bar{v}}$ and $E[N](\Bar{k})$ are not isomorphic unless $A$ is trivial. 
		
		(If the class $A$ is fixed, I hope that the number of elliptic curves for which $(H^{\mathrm{ab}}_{\Bar{v}})^{\Gamma_{k'}}=0$ is still finite.)
	\end{proof}
